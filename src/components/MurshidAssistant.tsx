...
import { performRAG } from "@/services/ai-integration";
...
export default function MurshidAssistant({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const [studentName, setStudentName] = useState<string>(() => {
    return localStorage.getItem("murshid_student_name") || "";
  });
  const [hasGreeted, setHasGreeted] = useState(false);
  const scrollRef = useRef<HTMLDivElement>(null);
  const chatRef = useRef<HTMLDivElement>(null);
  const isDragging = useRef(false);
  const [assistantAllowed, setAssistantAllowed] = useState(true);
  ...
  useEffect(() => {
    if (user) {
      ...
      if (name && name !== studentName) {
        setStudentName(name);
        localStorage.setItem("murshid_student_name", name);
      }
    }
  }, [user]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const aiResponse = await performRAG(input, studentName, isFirst);
                              ...
                              <div className="mt-3 flex flex-wrap gap-1.5 items-center">
                                {s.suggestions.map((suggest: string) => (
                                  <button
                                    ...
                                    onClick={() => {
                                      setInput(suggest);
                                      setTimeout(() => document.getElementById('murshid-send-btn')?.click(), 50);
                                    }}
