...
const StaticBackground = memo(() => {
  return (
    <div className="pointer-events-none fixed inset-0 overflow-hidden -z-10 bg-background gpu-accelerated" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', perspective: 1000, WebkitPerspective: 1000 }}>
      ...
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_50%_0%,hsl(var(--accent)/0.02)_0%,transparent_70%)]" />

      {/* Subtle Static Glows - Enabled only for Desktop */}
      <div
        style={{ willChange: "transform", background: "radial-gradient(circle, hsl(var(--accent)/0.03) 0%, transparent 70%)" }}
        className="absolute -top-40 -right-40 h-[45rem] w-[45rem] rounded-full hidden md:block"
      />
      <div
        style={{ willChange: "transform", background: "radial-gradient(circle, hsl(var(--primary)/0.04) 0%, transparent 70%)" }}
        className="absolute -bottom-60 -left-40 h-[50rem] w-[50rem] rounded-full hidden md:block"
      />

      {/* Glassmorphic Accents - Optimized */}
      <div className="absolute top-[15%] right-[10%] w-32 h-32 rounded-3xl bg-foreground/[0.01] border border-foreground/5 hidden lg:block" />
      <div className="absolute bottom-[20%] left-[15%] w-48 h-48 rounded-full bg-accent/[0.005] border border-accent/5 hidden lg:block" />
