// BAU 4.0 grading scale based on the new system.
...
export function calcGpa(prevCgpa: number, prevHours: number, courses: CourseEntry[]): GpaResult {
  let semPoints = 0;
  let semHours = 0;
  let adjustedPrevPoints = (prevCgpa || 0) * (prevHours || 0);
  let adjustedPrevHours = prevHours || 0;
  ...
  for (const c of courses) {
    ...
    if (c.isRepeat) {
      const gpOld = c.oldGrade ? gradePoints[c.oldGrade] : 0;
      adjustedPrevPoints -= (gpOld || 0) * c.hours;
      adjustedPrevHours -= c.hours;
    }
  }

  const semesterGpa = semHours > 0 ? semPoints / semHours : 0;
  const totalHours = Math.max(0, adjustedPrevHours) + semHours;
  const newCgpa = totalHours > 0 ? (Math.max(0, adjustedPrevPoints) + semPoints) / totalHours : 0;
