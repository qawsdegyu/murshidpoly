...
function CoursePageDetail() {
  ...
  useEffect(() => {
    ...
    async function fetchCourseData() {
      ...
      try {
        ...
        const mappedCourse = {
          ...courseData,
          nameAr: courseData.name_ar || courseData.nameAr || courseData.name_en || "",
          name: courseData.name_en || courseData.nameEn || courseData.name_ar || "",
          department: courseData.department || "",
          code: courseData.code || "",
          hours: courseData.credit_hours || courseData.hours || 3,
        ...
        if (active) {
          ...
          toast.error(isAr ? "خطأ في تحميل بيانات المادة" : "Error loading course data");
        }
      } finally {
        if (active) {
          setIsLoading(false);
  ...
  const getFilteredResources = () => {
    switch (activeTab) {
      case 'videos': return resources.filter(r => (r.category === "video" || r.type === "video"));
      case 'summaries': return resources.filter(r => (r.category === "summary" || r.type === "summary"));
      case 'pastPapers': return resources.filter(r => (r.category === "exam" || r.type === "exam"));
      case 'textbook': return resources.filter(r => (r.category === "book" || r.type === "book"));
      default: return [];
    }
  };
