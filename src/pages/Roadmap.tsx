...
function RoadmapDetail() {
  ...
  const [roadmapList, setRoadmapList] = useState<RoadmapNode[]>([]);
    ...
    async function fetchRoadmap() {
      ...
      try {
        ...
        if (data && data.length > 0) {
          // Map snake_case to camelCase if necessary (matching RoadmapNode interface)
          const mapped = data.map(node => ({
            ...node,
            fallbackName: node.fallback_name,
            fallbackNameAr: node.fallback_name_ar
          }));
          setRoadmapList(mapped as RoadmapNode[]);
        } else {
          setRoadmapList(roadmapNodes);
        }
      } catch (err) {
  ...
  const activeRoadmap = useMemo(() => {
    return roadmapList.filter(node => {
  ...
  return (
    <div className="min-h-screen pb-20 overflow-hidden bg-background selection:bg-cyan-500/30 transition-colors duration-300">
      ...
      <div className="relative z-10 max-w-[1440px] mx-auto px-4 md:px-8 pt-28">
            ...
            return (
              <motion.button
                ...
                className={cn(
                  ...
                  isActive
                    ? "bg-accent/10 border-accent/50 text-accent shadow-[0_0_20px_rgba(34,211,238,0.2)]"
                    : "bg-card/80 border-border shadow-sm backdrop-blur-xl text-muted-foreground hover:text-foreground"
                )}
              >
                {isActive && (
                  <motion.div
                ...
                <Icon className={cn("h-4 w-4 md:h-5 md:w-5", isActive ? "text-accent" : "text-muted-foreground group-hover:text-accent/80")} />
        ...
        <div className="relative h-[60vh] md:h-[calc(100vh-250px)] min-h-[400px] md:min-h-[750px] w-full rounded-2xl md:rounded-[2.5rem] border border-border bg-surface/40 backdrop-blur-3xl overflow-hidden shadow-2xl transition-colors duration-300">
          {/* Controls HUD */}
          ...
          <div className="absolute bottom-6 right-6 z-40 flex items-center gap-4 bg-surface/80 border border-border backdrop-blur-xl px-6 py-3 rounded-2xl">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-cyan-400 shadow-[0_0_8px_#22d3ee]" />
              <span className="text-[10px] font-black text-foreground/80 uppercase tracking-tighter">{lang === "ar" ? "متطلب سابق" : "Prerequisite"}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-rose-500 shadow-[0_0_8px_#f43f5e]" />
              <span className="text-[10px] font-black text-foreground/80 uppercase tracking-tighter">{lang === "ar" ? "متطلب متزامن" : "Co-requisite"}</span>
            </div>
          </div>
        ...
        {selectedNode && courseData && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-card/90 border border-border rounded-[3rem] overflow-hidden shadow-2xl backdrop-blur-2xl"
            >
              <div className="bg-gradient-to-br from-cyan-600/30 to-blue-900/40 p-10 relative">
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="text-[10px] font-bold text-muted-foreground tracking-widest uppercase">{courseData.hours} CREDIT HOURS</div>
                </div>
                <h3 className="text-4xl font-black text-foreground leading-tight mb-2">
  ...
  return (
    <Component
      ...
      className={cn(
        ...
        isActive || isHighlighted
          ...
          : "bg-surface border-border hover:border-border/80 shadow-xl"
      )}
      style={{
        left: `${xPos}px`,
        top: `${yPos}px`,
    ...
    >
      ...
      <div className="flex flex-col items-center gap-3 text-center relative z-10">
        ...
        <div className="text-base md:text-lg font-black text-foreground group-hover:text-accent group-hover:scale-[1.02] transition-all leading-tight">
...
function GlassModalLink({ icon, label, count, to }: any) {
  return (
    <Link to={to} className="flex flex-col gap-3 p-6 rounded-3xl bg-surface border border-border hover:bg-surface/80 transition-all group overflow-hidden relative">
      <div className="absolute top-0 right-0 p-1 opacity-10 group-hover:opacity-20 transition-opacity">
        {icon}
      </div>
      <div className="h-10 w-10 rounded-2xl bg-surface/50 flex items-center justify-center text-xl">
        {icon}
      </div>
      <div>
        <div className="font-black text-sm text-foreground mb-1">{label}</div>
        <div className="text-[10px] font-bold text-muted-foreground tracking-widest">{count} DOCUMENTS</div>
      </div>
    </Link>
  );
}
...
function ControlButton({ onClick, icon, label }: any) {
  return (
    <button
      onClick={onClick}
      className="p-3 rounded-xl bg-surface/80 border border-border backdrop-blur-xl hover:bg-surface transition-all text-muted-foreground hover:text-foreground group relative shadow-sm"
      title={label}
    >
      {icon}
      <span className="absolute right-full mr-3 px-2 py-1 rounded bg-foreground text-[10px] text-background opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
        {label}
      </span>
    </button>
  );
...
function DetailAction({ icon, label, count }: any) {
  return (
    <div className="p-8 rounded-[3rem] bg-white/5 border border-white/5 space-y-6 group hover:bg-white/10 transition-colors">
      <div className="w-16 h-16 rounded-[1.5rem] bg-primary/10 flex items-center justify-center text-primary shadow-inner group-hover:scale-110 transition-transform">
        {icon}
      </div>
      <div>
        <div className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground mb-1">{label}</div>
        <div className="text-3xl font-black">{count} <span className="text-lg opacity-40 font-bold">Files</span></div>
      </div>
    </div>
  );
}
