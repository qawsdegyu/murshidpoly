    ...
    try {
      ...
      if (res.success && res.suggestions && res.suggestions.length > 0) {
        const options: ScheduleOption[] = res.suggestions.slice(0, 5).map((s: any) => {
          ...
          activePlan.forEach(planCourse => {
            ...
            if (!hasSection) {
              // Force-find using any possible match
          ...
          // 3. DEDUPLICATION: Ensure exactly ONE section per course
      ...
      if (rawSchedules.length === 0) {
        ...
        const options: ScheduleOption[] = rawSchedules.slice(0, 7).map((opt, idx) => {
          ...
          // MANDATORY SYNC for local fallback
          activePlan.forEach(planCourse => {
            const hasSection = selectedSections.some(sec =>
          ...
          // 2. DEDUPLICATION: Ensure exactly ONE section per course
          const finalSections: Section[] = [];
          const seenCourseCodes = new Set<string>();
          selectedSections.forEach(sec => {
            const course = dbCourses.find(c => c.id === sec.courseId || (c.code && normalizeCode(c.code) === normalizeCode(sec.courseId)));
          ...
          <div className="p-6 rounded-[2rem] bg-surface/40 backdrop-blur-xl border border-border/50 shadow-elegant">
            <div className="space-y-5">
              ...
              <div className="space-y-3 pb-5 border-b border-border/10">
                ...
                <div className="grid grid-cols-3 gap-2 p-1.5 bg-primary/5 rounded-xl border border-primary/10">
                  <button onClick={() => setAiIntent('relaxed')} className={cn("py-3 text-sm sm:text-base font-black rounded-lg transition-all", aiIntent === 'relaxed' ? "bg-primary text-white shadow-md" : "text-primary/60 hover:bg-primary/5")}>{isAr ? "مريح" : "Relaxed"}</button>
                  <button onClick={() => setAiIntent('balanced')} className={cn("py-3 text-sm sm:text-base font-black rounded-lg transition-all", aiIntent === 'balanced' ? "bg-primary text-white shadow-md" : "text-primary/60 hover:bg-primary/5")}>{isAr ? "متوازن" : "Balanced"}</button>
                  <button onClick={() => setAiIntent('compressed')} className={cn("py-3 text-sm sm:text-base font-black rounded-lg transition-all", aiIntent === 'compressed' ? "bg-primary text-white shadow-md" : "text-primary/60 hover:bg-primary/5")}>{isAr ? "مكثف" : "Fast"}</button>
                </div>
              </div>

              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-black text-muted-foreground uppercase block px-1">{isAr ? "التخصص المعتمد" : "Selected Major"}</label>
                  <div className="w-full bg-accent/5 border border-accent/20 rounded-xl px-5 py-4 flex items-center justify-between group">
                    <span className="text-base font-black text-accent">
                      {majors.find(m => m.id === selectedMajor)?.[isAr ? 'nameAr' : 'name'] || selectedMajor}
                    </span>
                    <button
                      onClick={() => navigate('/settings')}
                      className="text-xs font-black text-muted-foreground hover:text-accent underline transition-colors"
                    >
                      {isAr ? "تغيير" : "Change"}
                    </button>
                  </div>
                  <p className="text-[11px] text-muted-foreground font-bold px-1 italic">
          ...
          <div className="p-6 rounded-[2rem] bg-surface/40 backdrop-blur-xl border border-border/50 shadow-elegant flex flex-col min-h-[500px] lg:max-h-[calc(100vh-4rem)] overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between mb-6 px-1">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-black uppercase tracking-widest text-foreground">{isAr ? "المواد المختارة" : "Selected"}</h3>
              </div>
              <span className="bg-accent/10 text-accent text-sm font-black px-3 py-1 rounded-lg">{activePlan.length}</span>
            </div>

            <div className="flex gap-2 mb-6 p-1.5 bg-background/30 rounded-xl border border-border/20">
              <button onClick={() => setMode("plan")} className={cn("flex-1 py-3 text-base font-black rounded-lg transition-all", mode === "plan" ? "bg-accent text-accent-foreground shadow-md" : "text-muted-foreground")}>{isAr ? "الخطة" : "Plan"}</button>
              <button onClick={() => setMode("manual")} className={cn("flex-1 py-3 text-base font-black rounded-lg transition-all", mode === "manual" ? "bg-accent text-accent-foreground shadow-md" : "text-muted-foreground")}>{isAr ? "يدوي" : "Manual"}</button>
            </div>

            {mode === "plan" && (
              <div className="flex-1 overflow-y-auto space-y-2.5 px-0.5 custom-scrollbar mb-4">
                {recommendedCourses.map(course => {
                  // Triple-match selection check for sidebar
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-end">
                        <span className={cn("text-base sm:text-lg font-black transition-colors", isSelected ? "text-accent" : "text-foreground")}>
                          {isAr ? course.nameAr : course.name}
                        </span>
                        <span className={cn("text-sm font-bold transition-colors", isSelected ? "text-accent/60" : "text-muted-foreground")}>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 bg-background/50 border border-accent/20 rounded-2xl p-1.5">
                    <button
                      ...
                      className="p-2 rounded-xl hover:bg-surface transition-all disabled:opacity-20 text-accent"
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="flex items-center justify-center w-12 h-12 bg-accent/10 text-accent rounded-full hover:bg-accent hover:text-accent-foreground transition-all shadow-md active:scale-95"
                    ...
                    <div className="bg-[#f9f3d5] rounded-lg border-2 border-[#b0b0b0] shadow-md text-black font-sans relative overflow-hidden rtl:text-right">
                      ...
                      <div className="bg-[#f9f3d5] p-6 border-b-2 border-[#c0c0c0] flex justify-between items-start">
                        <div className="w-32 flex justify-start">
                          <img src="/logo-dark.svg" alt="Hadeed Logo" className="h-16 w-auto object-contain" />
                        </div>
