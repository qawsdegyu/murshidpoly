...
import {
  ...
  Settings2, Layers, Info, Maximize2, Minimize2, Loader2, Table2, LayoutGrid, Edit, AlertCircle, X, Layout
} from "lucide-react";
        ...
        <div className="lg:col-span-3 xl:col-span-3 space-y-5 lg:order-1">
          ...
          <div className="p-6 rounded-[2rem] bg-surface/40 backdrop-blur-xl border border-border/50 shadow-elegant flex flex-col min-h-[500px] lg:max-h-[calc(100vh-4rem)] overflow-y-auto custom-scrollbar">
            ...
            <div className="mt-4 pt-4 border-t border-border/20 space-y-4">
              <div className="flex justify-between items-center px-1">
                <span className="text-sm font-black uppercase text-muted-foreground/80">{isAr ? "إجمالي الساعات" : "Total Hours"}</span>
                <span className={cn("text-xl font-black", totalHours > maxHours ? "text-destructive" : "text-accent")} dir="ltr">{totalHours} / {maxHours}h</span>
              </div>
        ...
        <div className="lg:col-span-9 xl:col-span-9 space-y-4 lg:order-2">
          ...
          <div className="flex items-center justify-between p-5 bg-surface/50 backdrop-blur-xl border border-border/50 rounded-[2rem] shadow-elegant mb-6">
            <div className="flex items-center gap-6">
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase text-accent/60 mb-1">{isAr ? "الخيار الحالي" : "CURRENT OPTION"}</span>
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-1 bg-background/50 border border-border/20 rounded-xl p-1">
                    <button
                      onClick={() => setCurrentOptionIndex(prev => Math.max(0, prev - 1))}
                      className="p-1.5 rounded-lg hover:bg-surface transition-all disabled:opacity-20"
                      disabled={currentOptionIndex === 0}
                    >
                      <ChevronRight className="w-4 h-4" />
                    </button>
                    <span className="text-sm font-black min-w-[3.5ch] text-center text-accent">
            ...
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-center border-r border-border/20 pr-4 mr-2 rtl:border-r-0 rtl:border-l rtl:pr-0 rtl:pl-4 rtl:mr-0 rtl:ml-2">
                <span className="text-[10px] font-black uppercase text-muted-foreground mb-1">{isAr ? "إجمالي الساعات" : "TOTAL HOURS"}</span>
                <span className="text-xl font-black text-accent">{generatedSchedules[currentOptionIndex]?.totalHours || 0}</span>
              </div>
