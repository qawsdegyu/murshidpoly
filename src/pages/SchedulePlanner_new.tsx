...
import { getSmartScheduleRecommendations } from "@/services/ai-integration";
...
export default function SchedulePlanner() {
  ...
  const [mode, setMode] = useState<"plan" | "manual" | "suggested">("plan");
  ...
  const [dbSuggestedSchedules, setDbSuggestedSchedules] = useState<any[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const [coursesRes, sectionsRes, suggestedRes] = await Promise.all([
          supabase.from('courses').select('*'),
          supabase.from('sections').select('*'),
          supabase.from('suggested_schedules').select('*')
        ]);
        ...
        if (suggestedRes.data) {
          setDbSuggestedSchedules(suggestedRes.data);
        }
      } catch (err) {
  ...
  const recommendedCourses = useMemo(() => {
    ...
    const filtered = dbCourses.filter(c =>
      roadmapForMajor.some(rn => rn.id === c.id || rn.id === c.code || rn.fallbackNameAr === c.nameAr || rn.fallbackName === c.name) &&
      !activePlan.find(ap => ap.id === c.id)
    );
    console.log("Debug Schedule:", {
      dbCoursesCount: dbCourses.length,
      roadmapForMajorCount: roadmapForMajor.length,
      selectedLevel,
      targetCategory,
  ...
  const handleApplySuggested = (suggested: any) => {
    const sectionsToApply = dbSections.filter(s => suggested.section_ids.includes(s.id));
    if (sectionsToApply.length === 0) {
      toast.error(isAr ? "لم يتم العثور على شعب هذا الجدول في قاعدة البيانات" : "No sections found for this schedule in DB");
      return;
    }

    const option: ScheduleOption = {
      sections: sectionsToApply,
      totalHours: sectionsToApply.reduce((sum, s) => {
        const c = dbCourses.find(course => course.id === s.courseId);
        return sum + (c?.hours || 0);
      }, 0)
    };

    setGeneratedSchedules([option]);
    setCurrentOptionIndex(0);
    setViewMode("table");
    toast.success(isAr ? "تم تطبيق الجدول المقترح" : "Suggested schedule applied");

    // Also update active plan to match
    const coursesInSuggested = dbCourses.filter(c => sectionsToApply.some(s => s.courseId === c.id));
    setActivePlan(coursesInSuggested);
  };

  const handleAiMagic = async () => {
    setIsAiGenerating(true);
    setAiSuggestions([]);
    toast.info(isAr ? "جاري استشارة الذكاء الاصطناعي..." : "Consulting AI...");

    try {
      const res = await getSmartScheduleRecommendations(
        selectedMajor,
        selectedYear,
        semesterType === "summer" ? 3 : selectedSemester,
        dbSections,
        dbCourses,
        targetHours,
        aiIntent
      ...
      if (res.success && res.suggestions) {
        ...
        toast.success(isAr ? "تم توليد عدة خيارات ذكية!" : "Generated multiple AI options!");
      } else {
  ...
  const filteredSearch = useMemo(() => {
    ...
    // 2. Exist in the roadmap for this major (or common)
    // 3. HAVE at least one section in the database (The Journal)
    return dbCourses.filter(c => {
      ...
      const inRoadmap = roadmapNodes.some(rn =>
        (rn.id === c.code || rn.id === c.id) &&
        (rn.category === targetCategory || rn.category === "common")
      );

      const hasSections = dbSections.some(s => s.courseId === c.id);

      return matchesQuery && inRoadmap && hasSections && !activePlan.find(ap => ap.id === c.id);
    });
  }, [searchQuery, activePlan, dbCourses, dbSections, targetCategory]);
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-[1700px]">
      <PageHeader
        title={t.nav.planner.title}
        subtitle={t.nav.planner.subtitle}
        ...
        <div className="lg:col-span-3 xl:col-span-3 space-y-5 lg:order-1">
          ...
          <div className="p-6 rounded-[2rem] bg-surface/40 backdrop-blur-xl border border-border/50 shadow-elegant flex flex-col h-[500px]">
            ...
            <div className="flex gap-2 mb-4 p-1 bg-background/30 rounded-xl border border-border/20">
              <button onClick={() => setMode("plan")} className={cn("flex-1 py-1.5 text-xs font-black rounded-lg transition-all", mode === "plan" ? "bg-accent text-accent-foreground shadow-sm" : "text-muted-foreground")}>{t.nav.planner.tabs.plan}</button>
              <button onClick={() => setMode("manual")} className={cn("flex-1 py-1.5 text-xs font-black rounded-lg transition-all", mode === "manual" ? "bg-accent text-accent-foreground shadow-sm" : "text-muted-foreground")}>{t.nav.planner.tabs.manual}</button>
              <button onClick={() => setMode("suggested")} className={cn("flex-1 py-1.5 text-xs font-black rounded-lg transition-all", mode === "suggested" ? "bg-accent text-accent-foreground shadow-sm" : "text-muted-foreground")}>{t.nav.planner.tabs.suggested}</button>
            </div>

            {mode === "suggested" && (
              <div className="space-y-3 mb-4 overflow-y-auto max-h-[400px] no-scrollbar">
                {/* AI Intent Selector */}
                <div className="grid grid-cols-3 gap-1 p-1 bg-primary/5 rounded-xl border border-primary/10 mb-2">
                  <button onClick={() => setAiIntent('relaxed')} className={cn("py-1 text-[10px] font-black rounded-lg transition-all", aiIntent === 'relaxed' ? "bg-primary text-white" : "text-primary/60")}>{isAr ? "مريح" : "Relaxed"}</button>
                  <button onClick={() => setAiIntent('balanced')} className={cn("py-1 text-[10px] font-black rounded-lg transition-all", aiIntent === 'balanced' ? "bg-primary text-white" : "text-primary/60")}>{isAr ? "متوازن" : "Balanced"}</button>
                  <button onClick={() => setAiIntent('compressed')} className={cn("py-1 text-[10px] font-black rounded-lg transition-all", aiIntent === 'compressed' ? "bg-primary text-white" : "text-primary/60")}>{isAr ? "مكثف" : "Fast"}</button>
                </div>

                {/* AI Magic Button */}
                <button
                  onClick={handleAiMagic}
                  disabled={isAiGenerating || isDataLoading}
                  className="w-full relative overflow-hidden group p-4 rounded-2xl bg-gradient-to-br from-primary/20 via-primary/5 to-transparent border-2 border-primary/30 hover:border-primary/60 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                >
                  ...
                  <div className="flex items-center justify-between relative z-10">
                    <div className="text-right">
                      <h4 className="text-sm font-black text-primary flex items-center gap-2">
                        ...
                        {isAr ? "سحر الذكاء الاصطناعي" : "AI Magic Mode"}
                      </h4>
                      <p className="text-[10px] font-bold text-muted-foreground mt-0.5">
                        {isAr ? `توليد جداول بنمط: ${aiIntent === 'relaxed' ? 'مريح' : aiIntent === 'balanced' ? 'متوازن' : 'مكثف'}` : `Generating ${aiIntent} options`}
                      </p>
                    </div>
                    {isAiGenerating && <Loader2 className="w-4 h-4 animate-spin text-primary" />}
                  </div>
                </button>
                ...
                <AnimatePresence>
                  {aiSuggestions.map((s, idx) => (
                    <motion.button
                      ...
                      key={idx}
                      onClick={() => handleApplySuggested({ section_ids: s.sectionIds, title_ar: s.titleAr, title_en: s.titleEn })}
                      className="w-full text-right p-3 rounded-xl bg-primary/5 border border-primary/10 hover:bg-primary/10 transition-all group relative overflow-hidden"
                    >
                      <div className="absolute left-0 top-0 bottom-0 w-1 bg-primary" />
                      <div className="flex items-center justify-between mb-1">
                        <Sparkles className="w-3 h-3 text-primary" />
                        <span className="text-[10px] font-black uppercase text-primary/60">{isAr ? "خيار ذكي" : "AI OPTION"}</span>
                      </div>
                      <h4 className="text-sm font-black text-primary">{isAr ? s.titleAr : s.titleEn}</h4>
                      <p className="text-[10px] font-medium text-muted-foreground mt-1 line-clamp-2 italic">
                        "{isAr ? s.descriptionAr : s.descriptionEn || s.descriptionAr}"
                      </p>
                    </motion.button>
                  ))}
                </AnimatePresence>
                ...
                {dbSuggestedSchedules
                  .filter(s => s.major === selectedMajor && s.year === selectedYear && s.semester === (semesterType === "summer" ? 3 : selectedSemester))
                  .map(s => (
                    <button
                      key={s.id}
                      onClick={() => handleApplySuggested(s)}
                      className="w-full text-right p-3 rounded-xl bg-accent/5 border border-accent/10 hover:bg-accent/10 transition-all group"
                    >
                      <div className="flex items-center justify-between mb-1">
                        <Sparkles className="w-3 h-3 text-accent" />
                        <span className="text-[10px] font-black uppercase text-accent/60">{isAr ? "جدول جاهز" : "READY PLAN"}</span>
                      </div>
                      <h4 className="text-sm font-black">{isAr ? s.title_ar : s.title_en}</h4>
                      <p className="text-[10px] font-bold text-muted-foreground mt-1">
                        {isAr ? `${s.section_ids.length} شعب مختارة بعناية` : `${s.section_ids.length} handpicked sections`}
                      </p>
                    </button>
                  ))}

                {dbSuggestedSchedules.filter(s => s.major === selectedMajor && s.year === selectedYear && s.semester === (semesterType === "summer" ? 3 : selectedSemester)).length === 0 && (
                  <div className="py-10 text-center opacity-30">
                    <LayoutGrid className="w-8 h-8 mx-auto mb-2" />
                    <p className="text-xs font-bold">{isAr ? "لا توجد جداول مقترحة لهذا الفصل" : "No suggested schedules for this semester"}</p>
                  </div>
                )}
              </div>
            )}
